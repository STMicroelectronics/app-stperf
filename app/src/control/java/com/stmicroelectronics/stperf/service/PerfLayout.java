...
public class PerfLayout {
    ...
    private boolean mCpuFreqSwitchVisible;
    ...
    private CustomButton mSwitchFreqButton;
    private final WindowManager.LayoutParams mSwitchLayoutParams;

    private double mGraphIndex;
    private boolean mCpuAvg;

    private final Object key = new Object();
    private final CustomButton.OnClickListener mListener;

    private boolean mMoving;
    private int mPrevX, mPrevY;


    PerfLayout(Context context, boolean graph, boolean pos, boolean cpu_freq_switch, CustomButton.OnClickListener listener) {
        mContext = context;
        mGraph = graph;
        mPos = pos;
        mCpuFreqSwitchVisible = cpu_freq_switch;
        mListener = listener;
        mGraphIndex = 0;
        ...
        mSwitchLayoutParams = initSwitchLayoutParams(mPos);
        initButtonSwitchFreq();
        if (mCpuFreqSwitchVisible) {
            mWindowManager.addView(mSwitchFreqButton, mSwitchLayoutParams);
        }
    }
    ...
    void stopLayout() {
        ...
        if (mCpuFreqSwitchVisible) {
            mWindowManager.removeView(mSwitchFreqButton);
        }
    }
    ...
    private WindowManager.LayoutParams initSwitchLayoutParams(boolean pos) {
        WindowManager.LayoutParams layoutParams = new WindowManager.LayoutParams();
        layoutParams.type = WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY;
    ...
    private void initButtonSwitchFreq() {
        mSwitchFreqButton = new CustomButton(mContext, mListener);

        mSwitchFreqButton.setText(R.string.switch_freq);
        mSwitchFreqButton.setTextColor(Color.WHITE);
        mSwitchFreqButton.setPadding(20,20,20,20);
        mSwitchFreqButton.setBackgroundResource(R.drawable.custom_btn_grey);

        mSwitchFreqButton.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    ...
                    Timber.d("ACTION_DOWN with x=%d and y =%d", (int)event.getRawX(), (int)event.getRawY());

                } else if (event.getAction() == MotionEvent.ACTION_MOVE) {

                    WindowManager.LayoutParams params = (WindowManager.LayoutParams) mSwitchFreqButton.getLayoutParams();

                    Timber.d("ACTION_MOVE with x=%d and y =%d - params.x=%d and params.y=%d", (int)event.getRawX(), (int)event.getRawY(), params.x, params.y);

                    params.x = params.x - (int)(mPrevX - event.getRawX());
                    params.y = params.y + (int)(mPrevY - event.getRawY());

                    mPrevX = (int) event.getRawX();
                    mPrevY = (int) event.getRawY();

                    mWindowManager.updateViewLayout(mSwitchFreqButton, params);
    ...
    void updateSwitchLayoutVisibility(boolean cpu_freq_switch) {
        if (mCpuFreqSwitchVisible) {
            mWindowManager.removeView(mSwitchFreqButton);
        }
        mCpuFreqSwitchVisible = cpu_freq_switch;
        if (mCpuFreqSwitchVisible) {
            initButtonSwitchFreq();
            if (mPos) {
                mSwitchLayoutParams.gravity = Gravity.CENTER | Gravity.BOTTOM;
            } else {
                mSwitchLayoutParams.gravity = Gravity.CENTER | Gravity.TOP;
            }
            mWindowManager.addView(mSwitchFreqButton, mSwitchLayoutParams);
        }
    }
